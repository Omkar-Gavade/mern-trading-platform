...
//  How BUY avg price calculation works
  ...
  try {
    ...
    if (mode === "BUY") {
      if (!existing) {
        ...
        // Update holding with new average
      ...
      return res.json({ success: true, message: "BUY order updated holdings" });
    }
    ...
    if (mode === "SELL") {
      ...
      return res.json({ success: true, message: "SELL order updated holdings" });
    }
...
app.get("/holdings", async (req, res) => {
  try {
    const holdings = await HoldingsModel.find(); // Make sure you imported the model
    res.json(holdings);
  } catch (err) {
